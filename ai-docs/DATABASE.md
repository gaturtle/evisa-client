...
| CompanyName         | string?   | Nullable — sponsoring/host company in Vietnam; required client-side only when `PurposeOfTravel` is `"Business"` or `"Working"` |
| CompanyPhone        | string?   | Nullable — same conditional requirement as `CompanyName`                                                        |
| CompanyAddress      | string?   | Nullable — same conditional requirement as `CompanyName`                                                        |
...
| EmergencyContactName         | string? | Nullable — Extra Details field, see below                                                              |
| EmergencyContactPhone        | string? | Nullable — Extra Details field                                                                          |
| EmergencyContactRelationship | string? | Nullable — Extra Details field                                                                          |
| EmergencyContactAddress      | string? | Nullable — Extra Details field                                                                          |
| OccupationCompanyName        | string? | Nullable — Extra Details field                                                                          |
| OccupationJobTitle           | string? | Nullable — Extra Details field                                                                          |
| OccupationCompanyPhone       | string? | Nullable — Extra Details field                                                                          |
| OccupationCompanyAddress     | string? | Nullable — Extra Details field                                                                          |
| VnStayAddress                | string? | Nullable — Extra Details field                                                                          |
| VnStayPhone                  | string? | Nullable — Extra Details field                                                                          |
| VnVisitedLastYear            | string? | Nullable — Extra Details field; `"yes"` \| `"no"`                                                      |
| VnVisitDetails               | string? | Nullable — Extra Details field                                                                          |
| VnHasRelatives               | string? | Nullable — Extra Details field; `"yes"` \| `"no"`                                                      |
| VnRelativeDetails            | string? | Nullable — Extra Details field                                                                          |
...
> The `EmergencyContact*` / `Occupation*` / `Vn*` columns are the "Extra Details" shared/application-level fields — required only when `VisaNationality.RequiresExtraDetails` is set for the application's shared nationality (resolved from the first applicant's `NationalityId`); enforced in `ApplicationService`, not by a database constraint. See `Applicant` below for the corresponding per-applicant Extra Details fields.
...
| Religion          | string? | Nullable — Extra Details field, see below                                                                                                                                                                                                                                            |
| PhoneInCountry    | string? | Nullable — Extra Details field                                                                                                                                                                                                                                                       |
| UsedOtherPassport | string? | Nullable — Extra Details field; `"yes"` \| `"no"`                                                                                                                                                                                                                                    |
| OtherPassportNumber | string? | Nullable — Extra Details field                                                                                                                                                                                                                                                     |
| ViolatedLaws      | string? | Nullable — Extra Details field; `"yes"` \| `"no"`                                                                                                                                                                                                                                    |
| ViolationDetails  | string? | Nullable — Extra Details field                                                                                                                                                                                                                                                       |

> Table name: `Applicants` · Index: `IX_Applicants_ApplicationId`
> `Religion` through `ViolationDetails` are the "Extra Details" per-applicant fields — required only when `VisaNationality.RequiresExtraDetails` is set for the application's shared nationality; enforced in `ApplicationService`, not by a database constraint.
...
| RequiresExtraDetails | bool | Whether applicants of this nationality must supply the Extra Details fields (see `VisaApplication`/`Applicant` above) on submit/update. Independent of `IsEligible`, `GroupId`, and any restriction/exclusion/exception — unrelated to `VisaTypeGroupRestriction`/`PendingReview` despite the similar "needs more scrutiny" flavor. |
| CreatedDateTime  | DateTime | Default: DateTime.Now                             |
| ModifiedDateTime | DateTime | Default: DateTime.Now                             |
...
### VisaProcessingNationalityPrice

A specific (VisaProcessing, Nationality) pair (Nationality Price Override) that replaces a processing option's default `Price` for applicants of that nationality. Independent of `VisaProcessingGroupExclusion`/`VisaProcessingNationalityException` — a price override has no effect on eligibility, and a nationality's exclusion/exception status has no effect on its price.
...
| VisaProcessingId | Guid     | Logical FK → VisaProcessing (different DbContext)       |
| NationalityId    | Guid     | Logical FK → VisaNationality (different DbContext)      |
| Price            | decimal  | The overridden price for this (VisaProcessing, Nationality) pair |
| CreatedDateTime  | DateTime | Default: DateTime.Now                                   |
| ModifiedDateTime | DateTime | Default: DateTime.Now — updated on upsert               |

> Table name: `VisaProcessingNationalityPrice` · Unique index on `(VisaProcessingId, NationalityId)` · Managed by `NationalityGroupContext`
...
- **VisaProcessing + VisaNationality → VisaProcessingNationalityPrice**: Composite, at most one price-override row per (VisaProcessing, Nationality) pair.
...
- **VisaApplication**: Top-level submission record. Stores FK references to `VisaType` and `VisaProcessing` by ID. Deleted application cascades to its `Applicant` and `Payment` rows. Optional `Company*` columns (Business/Working purpose) and shared/application-level Extra Details columns (`EmergencyContact*`/`Occupation*`/`Vn*`) live here alongside the base fields.
- **Payment**: One-to-one with `VisaApplication`. Tracks the Stripe PaymentIntent and its lifecycle (Pending → Succeeded/Failed).
- **Applicant**: Per-person record linked to a `VisaApplication`. Photo files stored on disk; relative paths in `PassportPhotoPath` / `PortraitPhotoPath`. Also holds the per-applicant Extra Details columns (`Religion` through `ViolationDetails`).
- **VisaNationality**: Exemption data stored inline as nullable `ExemptionDays`. `NULL` means no visa exemption. `GroupId` places it in at most one `NationalityGroup`. `RequiresExtraDetails` independently gates whether Extra Details fields are required for applications using this nationality.
...
- **VisaProcessing**: Owns its own default `Price` column. `VisaProcessingNationalityPrice` overrides it per nationality — independent of the exclusion/exception tables above.
...
| `NationalityGroupContext` | NationalityGroups, VisaTypeGroupRestrictions, VisaTypeNationalityExceptions, VisaProcessingGroupExclusions, VisaProcessingNationalityExceptions, VisaProcessingNationalityPrices | `Migrations/NationalityGroup/` |
...
erDiagram
    VisaApplication {
        ...
        string EmergencyContactAddress "nullable"
        string OccupationCompanyName "nullable"
        string OccupationJobTitle "nullable"
        string OccupationCompanyPhone "nullable"
        string OccupationCompanyAddress "nullable"
        string VnStayAddress "nullable"
        string VnStayPhone "nullable"
        string VnVisitedLastYear "nullable"
        string VnVisitDetails "nullable"
        string VnHasRelatives "nullable"
        string VnRelativeDetails "nullable"
        int Status
    ...
    Applicant {
        ...
        string PhoneInCountry "nullable"
        string UsedOtherPassport "nullable"
        string OtherPassportNumber "nullable"
        string ViolatedLaws "nullable"
        string ViolationDetails "nullable"
    }
    ...
    VisaProcessing ||--o{ VisaProcessingNationalityPrice : "prices"
    VisaNationality ||--o{ VisaProcessingNationalityPrice : "priced for"
